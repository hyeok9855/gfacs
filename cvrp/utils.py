...
def load_vrplib_dataset(n_nodes, k_sparse_factor, device, dataset_name="X", filename=None):
    ...
    for normed_demand, position, distance, name in vrplib_list:
        ...
        normed_dist = torch.tensor(normed_dist, device=device, dtype=torch.float32)
    ...
    for scale in [200, 500, 1000, 2000]:
        ...
        try:
            ...
            for instance in dataset:
                ...
                demands_torch = torch.tensor([0] + [d / capacity for d in demands], dtype=torch.float32)
                positions_torch = torch.tensor([depot_position] + positions, dtype=torch.float32)
