...
class ACO():
    ...
    def sample(self, invtemp=1.0, K=1, epsilon=None):
        if K > 1:
            n_ants = self.n_ants
            self.n_ants = K * n_ants

        paths, log_probs = self.gen_path(invtemp=invtemp, require_prob=True, epsilon=epsilon)  # type: ignore
    ...
    def gen_path(self, invtemp=1.0, require_prob=False, paths=None, epsilon=None):
        ...
        while not done:
            ...
            actions, log_probs = self.pick_move(prob_mat[prev], visit_mask, capacity_mask, require_prob, invtemp, selected, epsilon)
    ...
    def pick_move(self, dist, visit_mask, capacity_mask, require_prob, invtemp=1.0, guiding_node=None, epsilon=None):
        ...
        if epsilon is not None:
            # With probability epsilon, choose a random action among the sparsified edges (where dist > 1e-9)
