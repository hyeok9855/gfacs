...
class ACO():
    def __init__(
        ...
        self.minmax_cost_scaling = minmax_cost_scaling
    ...
    def sample(self, invtemp=1.0, inference=False, start_node=None, epsilon=None):
        if inference:
            ...
            paths, log_probs = self.gen_path(invtemp=invtemp, require_prob=True, start_node=start_node, epsilon=epsilon)
    ...
    def update_pheromone(self, paths, costs):
        ...
        if self.minmax_cost_scaling:
            deltas_min, deltas_max = deltas.min(), deltas.max()
            deltas = (deltas - deltas_min) / (deltas_max - deltas_min + 1e-10)
            delta_gb = (delta_gb - deltas_min) / (deltas_max - deltas_min + 1e-10)
    ...
    def gen_path(self, invtemp=1.0, require_prob=False, paths=None, start_node=None, epsilon=None):
        ...
        for i in range(self.problem_size - 1):
            if paths is None or require_prob:
                dist = (prob_mat[prev] ** invtemp) * mask
                dist = dist / dist.sum(dim=1, keepdim=True)  # This should be done for numerical stability
                # Make it simplex
                if (dist.sum(dim=1) == 0).any():
                    dist[dist.sum(dim=1) == 0] = 1 / dist.shape[1]
                # Epsilon-noisy exploration
                if epsilon is not None:
                    # With probability epsilon, choose a random action among the sparsified edges (where self.heuristic > 1e-9)
                    # With probability 1-epsilon, choose an action according to the probabilities
                    edge_mask = (dist > 1e-9).float()
                    n_edges = edge_mask.sum(dim=1, keepdim=True)
                    dist = epsilon * edge_mask / n_edges + (1 - epsilon) * dist
