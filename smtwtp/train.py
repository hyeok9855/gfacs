...
def train_instance(
    ...
    for _ in range(batch_size):
        ...
        costs, log_probs, sols = aco.sample(invtemp=invtemp)
        baseline = costs.mean() if shared_energy_norm else torch.tensor(0.0, device=DEVICE)
        advantages = baseline - costs

        if guided_exploration:
            sols_ls, costs_ls = aco.local_search(sols, costs, inference=False)
            baseline_ls = costs_ls.mean() if shared_energy_norm else torch.tensor(0.0, device=DEVICE)
            advantages_ls = baseline_ls - costs_ls
        ...
        backward_flow = weighted_advantages.detach() * beta
        ...
        if guided_exploration:
            ...
            backward_flow_ls = advantages_ls.detach() * beta
        ...
        if USE_WANDB:
            ...
            if guided_exploration:
                ...
                _logZ_ls_mean += logZ_ls  # type: ignore
        ##################################################
...
def infer_instance(model, instance, n_ants):
    ...
    aco = ACO(due_time, weights, processing_time, n_ants, heuristic=heu_mat, use_local_search=True, device=DEVICE)
