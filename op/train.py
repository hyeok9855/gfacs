...
def train_instance(
    ...
    for _ in range(batch_size):
        ...
        objs, log_probs, sols = aco.sample(invtemp=invtemp)
        advantages = objs - objs.mean()

        if guided_exploration:
            sols_ls, objs_ls = aco.local_search(sols, objs, inference=False)
            advantages_ls = objs_ls - objs_ls.mean()
        ...
        backward_flow = math.log(1 / 2) + weighted_advantages.detach() * beta
        ...
        if guided_exploration:
            ...
            backward_flow_ls = math.log(1 / 2) + advantages_ls.detach() * beta
        ...
        if USE_WANDB:
            ...
            _logZ_mean += logZ
            if guided_exploration:
                _train_mean_cost_ls += (-objs_ls).mean().item()  # type: ignore
...
def infer_instance(model, instance, n_ants):
    ...
    aco = ACO(distances, prizes, MAX_LEN[len(prizes)], n_ants, heuristic=heu_mat, use_local_search=True, device=DEVICE)
...
def train_epoch(
    ...
    for i in tqdm(range(steps_per_epoch), desc="Train", dynamic_ncols=True):
        ...
        train_instance(net, optimizer, batch_size, n_nodes, k_sparse, n_ants, cost_w, invtemp, guided_exploration, beta, it)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--val_size", type=int, default=20, help="Number of instances for validation")
    ...
    if args.beta_min is None:
        beta_min_map = {100: 5, 200: 5, 500: 10}
        args.beta_min = beta_min_map[args.nodes]
    if args.beta_max is None:
        beta_max_map = {100: 10, 200: 10, 500: 20}
